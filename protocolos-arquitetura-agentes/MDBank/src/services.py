import logging
# responsáveis pela request e pela identificação da requisição 
import httpx
import uuid
...
# -----------------------------
# REGISTRY DE AGENTES: agents no padrao A2A
# -----------------------------
AGENTS = {
...
# -----------------------------
# STATE DO LANGGRAPH: state local
# -----------------------------


class State(TypedDict):
    query: str
    responses: Annotated[list[str], add]  #vai ter mais de uma resposta, então é uma lista de strings; ele vai juntar e formar uma resposta única 

# -----------------------------
# EVENTO PARA STATE UPDATE: state pro front
# -----------------------------
...
async def request_agent(message: str, agent_url: str) -> str:
    ...
    event = await client.send_message(request)
...
# -----------------------------
# BUILD DO GRAFO: Construção do grafo de estados
# -----------------------------
...
# Função para executar o supervisor que invoca o grafo de estados
